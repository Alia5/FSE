#pragma once
// stolen from: https://gist.github.com/ddovod/be210315f285becc6b0e455b775286e1
...
namespace ImGui
{
		...
		bool cpu_fine_clip = false)
	{
		...
		pos.x = (float)(int)pos.x + font->ConfigData->GlyphOffset.x;
		pos.y = (float)(int)pos.y + font->ConfigData->GlyphOffset.y;
		float x = pos.x;
		float y = pos.y;
		if (y > clip_rect.w)
			return;
	...
	void TextAnsiUnformatted(const char* text, const char* text_end)
	{
		...
		if (text_end - text > 2000 && !wrap_enabled) {
			...
			if (text_pos.y <= clip_rect.Max.y) {
				...
				// Lines to render
				if (line < text_end) {
					ImRect line_rect(pos,  ImVec2(pos.x + FLT_MAX, pos.y + line_height));
					while (line < text_end) {
